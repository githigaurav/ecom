...
function Login() {
    ...
    const formik = useFormik({
        ...
        onSubmit: async(values) => { 
            ...
            try {
                const result = await auth("post", "user/login", values)
        ...
        />
            <div className='flex h-screen justify-center items-center w-full'>
                <div className='flex flex-col gap-3 w-full max-w-[600px]'>
                    <h3 className='m-5 uppercase font-bold text-4xl text-center text-blue-600'>Login</h3>
                    <Input
                        type={"text"}
                        placeholder={"Email"}
                    ...
                    <p className='text-center'>Don't have an account ? <Link to="/user/signup" className='text-blue-600 font-semi-bold'>Sign Up</Link> </p>
                </div>
            </div>
            </Animation>
        </>
